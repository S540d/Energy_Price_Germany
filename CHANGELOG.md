...
### Fixed
- **Nationale Erneuerbaren-Werte für morgen fehlten dauerhaft (#487).** Das Chart zeigte für morgen Preise, aber keine bundesweiten Erneuerbaren-Werte. Ursache war nicht die API — die liefert ab ca. 15 UTC volle 192 Punkte bis morgen 23:45 Berlin —, sondern das `gate` in `fetch.yml`: Sein Erneuerbaren-Kriterium zählte nur Punkte „für heute" und sah damit nie, dass Werte für morgen fehlen; das Preis-Kriterium griff ebenfalls nicht, weil die Datei via aWATTar bereits weiter reichende Preise enthielt (aWATTar setzt `renewable_share: null` per Design). Die Läufe um 16 und 19 UTC, die die erweiterte Prognose geholt hätten, entschieden dadurch täglich `run-fetch=false` — seit Einführung des Gates (#435) endeten die Erneuerbaren-Werte jeden Tag um 23:45 Berlin. Das Kriterium vergleicht jetzt Zeitstempel über das gesamte API-Fenster; die Logik liegt in `scripts/gate-decision.js` und ist über sieben Unit-Tests abgesichert.
...
- **Erneuerbaren-Ausfall wurde in der UI kaschiert statt angezeigt (Folgefall zu #417/#445).** Am 08.09.2026 fehlten die nationalen `ren_share_forecast`-Werte für den gesamten laufenden Tag (letzter Punkt: 07.09., 23:45; erst der Lauf um 15:34 UTC brachte sie zurück). Die App zeigte dazu drei Dinge gleichzeitig an: `--` in der Kachel, `Tages-Ø 0.0 %` darunter und eine plausibel wirkende Ø-Linie bei 45 % im Chart. Alle drei stammen aus derselben leeren Datenlage, sind aber unterschiedlich falsch:
  - **`Tages-Ø 0.0 %` war eine erfundene Zahl.** `calculateMetrics` gab bei null Datenpunkten `0` statt `null` zurück; „keine Daten" las sich damit als „keine Erneuerbaren im Netz". `today.renewable.{avg,min,max}` sind jetzt `number | null`, die Kachel blendet die Zeile bei `null` aus. Gleiches Muster in `MetricsView`: Statistikwerte zeigen `—`, der Energiemix-Bubble-Chart (der sonst „0 % erneuerbar / 100 % konventionell" behauptet hätte) entfällt ganz. Preise bleiben unverändert numerisch.
  - **Die Ø-Linie entstand aus einem Sechstel der Daten.** Sie mittelt über alle Werte im Chart-Fenster, unabhängig vom Tag — hier über die 28 verbliebenen Punkte vom Vorabend (Ø 46,1 %), gezogen über zwei Tage. `RenewableBarChart` zeichnet sie nur noch, wenn mindestens 50 % der Punkte im Fenster einen Wert haben; darunter erscheint stattdessen „Kein Ø: nur {valid} von {total} Werten".
  - **Ein gültiger Wert lag vor und wurde weggeworfen.** Nationale und regionale Erneuerbaren-Werte kommen aus zwei getrennten Quellen über zwei getrennte Wege (Workflow-Datei vs. Signal API live im Client) und fallen unabhängig voneinander aus — im Screenshot lief die regionale Linie durch, während die nationalen Balken abbrachen. `renewableShareRegional` wurde bisher nur gezeichnet, aber in keiner Kennzahl ausgewertet. Neu: fehlt der nationale Wert für heute vollständig, tritt ein Ortswert an seine Stelle — die eigene PLZ, sonst der Ersatzort aus der Country-Registry (DE: Berlin, `fallbackPostalCode`). **Der Wert wird immer als Ortswert gekennzeichnet** („⚠ nur Berlin, nicht bundesweit"), weil die Streuung zwischen Netzregionen regelmäßig über Faktor 2 liegt und ein Ortswert den Bundesmix nicht abbildet. Nationale Werte haben immer Vorrang; ein Mischen beider Quellen (nationaler Tages-Ø neben aktuellem Ortswert) findet bewusst nicht statt. Der Ersatzort wird nur für die Kachel abgerufen und nicht in die Chart-Daten gemergt — sonst erschiene eine Regionallinie für einen Ort, den niemand ausgewählt hat.

  Neue Unit-Tests: `utils/__tests__/renewableFallback.test.ts` (14) plus Regressionstests in `metrics.test.ts` und `RenewableBarChart.test.tsx`. Neue i18n-Keys (EN+DE): `averageLowCoverage`, `renewableFallbackOwn`, `renewableFallbackDefault`.
...
### Documentation
- **`CLAUDE.md` entrümpelt, Vorfallsarchiv `docs/INCIDENTS.md` (Retrospektive).** Der Git-/CI-Prozessteil war auf 390 Zeilen gewachsen und mischte geltende Regeln mit Vorfallsnarrativen — mit nachweislich falschen Passagen (ein nie existierender Autofix-Review, „Push scheitert mit 403" statt `GH013`, #428 als offen markiert obwohl gelöst, `github-actions[bot]` als Bypass-Actor empfohlen obwohl GitHub das nicht zulässt). Ein Regelwerk, dem man nicht trauen kann, kostet pro Fehlstelle eine Runde vergebliches Suchen. Jetzt getrennt: `CLAUDE.md` enthält knappe, handlungsleitende Regeln mit Prüfbefehl (**−33 % Textumfang** im Prozessteil), die Chronik samt Run-IDs, PR-Nummern und Messwerten steht in `docs/INCIDENTS.md` und ist von den Regeln aus verlinkt. Nichts gelöscht, nur verschoben — die Historie ist wertvoll, aber sie ist kein Regelwerk. `[GLOBAL POLICY]` unangetastet (wird aus project-templates synchronisiert).
